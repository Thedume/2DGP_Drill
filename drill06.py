...
def handle_events():
    ...
    for event in events:
        if event.type == SDL_QUIT:
            ...
            points.append((event.x, TUK_HEIGHT - 1 - event.y))  # 클릭된 위치를 새로운 점으로
...
def update_world():
    ...
    if target_exists:
        if t <= 1.0:
            cx = (1 - t) * sx + t * hx  # cx 는 시작 x 와 끝 x를 1-t:t의 비율로 섞은 위치
            cy = (1 - t) * sy + t * hy
            t += 0.001
        else:  # 목표 지점에 도달하면
            cx, cy = hx, hy  # 캐릭터의 위치를 기존 목적지 위치와 강제로 정확히 일치시킴
